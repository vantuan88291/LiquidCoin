...
export const WelcomeScreen: FC<WelcomeScreenProps> = observer(function WelcomeScreen() {
    ...
    return (
        <Screen statusBarStyle='light' safeAreaEdges={['top', 'bottom']} preset='fixed' style={$container}>
            <View style={$wrap}>
                <View style={$row}>
                    <TouchableOpacity onPress={setTab(0)} style={[$button, position === 0 && {borderBottomWidth: 2}]}>
                    <Text text={'Calculate Liquid'}/>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={setTab(1)} style={[$button, position === 1 && {borderBottomWidth: 2}]}>
                        <Text text={'Avg Entry'}/>
                    </TouchableOpacity>
                </View>
                <Swiper
