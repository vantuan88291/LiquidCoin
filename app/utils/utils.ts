...
function trimTraillingZeros(str: string, decimalSeparator = "."): string {
...
export function formatNumber(num: number | string, trun: number): string {
  ...
  const decimals =
    truncatedPost && truncatedPost.length > 0 ? `${decimalSeparator}${truncatedPost}` : ""
  formattedValue = `${pre.replace(rex, `$&${digitGroupingSeparator}`)}${decimals}`
  return formattedValue
}
export function formattedCurrency(num: number | string, tickSize = 4): string {
